...
public class CreateEntity {
    ...
    private final static SimpleDateFormat ft = new SimpleDateFormat ("yyyyMMddHHmm");

    private final static String titleName = "中药方剂资料库";
    private final static String DATE = titleName + "_" +ft.format(new Date());

    public static void main(String[] args) throws IOException {
        String className = "ChineseMedicinalFormulaeInfo";
        ...
        createConfigTxt(className, attributes);
        ...
        createSQL(className, attributes);
        createJSP(className);
        createJS(className, attributes);
    }

    private static void createConfigTxt(String className, String[][] attributes) {
        List<String> lines = new ArrayList<>();
        lines.add("String[][] attributes = {");
        for (String[] s1 : attributes) {
            lines.add(String.format("{\"%s\", \"%s\" },", s1[0], s1[1]));
        }
        lines.add("};");
        lines.add("String titleName = \"" + titleName + "\";");
        lines.add("String className = \"" + className + "\";");
        writeFile(lines, className + ".txt");
    }
    ...
    private static void createJS(String className, String[][] attributes) {
        ...
        lines.add(String.format("url: ctx + \"/query/%sService/list.do\",", serviceName));
        lines.add(String.format("editUrl: ctx + \"/%s/execute.do\",\n" + "reasonUrl: ctx + \"/%s/editLog.do\",",
                serviceName,
                serviceName));
        lines.add("enableColumnHide: true,");
        lines.add("inViewportShow: true,");
        lines.add("iconCls: \"table\",");

        // 写入列
        lines.add("columns: [");
        lines.add("{header: 'ID',dataIndex: 'ID',editname: 'id',hidden: true, hideable: false},");
        for (String[] att : attributes) {
            lines.add(String.format(
                    "{header: '%s',dataIndex: '%s',editname: '%s',hidden: false, hideable: true, editable: true, search: true, searchoptions: { sopt: ['cn', 'eq'] } },",
                    att[0],
     ...
     * @param className 类名
     */
    private static void createJSP(String className) {
        String codeName = getServiceName(className);
        List<String> lines = new ArrayList<>();
     ...
     */
    private static void createSQL(String className, String[][] attributes) {
        List<String> lines = new ArrayList<>();
        // CREATE TABLE
        ...
        // 增加列注释
        for (String[] att : attributes) {
            lines.add(String.format("COMMENT ON COLUMN %s.%s IS '%s';", getTableName(className), humpToUnderline(att[1]), att[0]));
        }
    ...
    private static void createController(String className, String pageName) {
        ...
        // 写入注解
        lines.add(String.format("@RestController\n" + "@RequestMapping(\"/%s\")", getServiceName(className)));
    ...
    private static void writeFile(List<String> lines, String fileName) {
        // 输出到文件
        try {
            FileUtils.writeLines(new File(String.format("D:\\package\\%s\\%s", DATE, fileName)), lines, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static String getTableName(String className) {
        return "KBMS" + humpToUnderline(className);
    }
