...
public final class AnnotationConfiguration {
  ...
  /**
   * 1. 如果属性的get or set 方法上没有增加<code>Column</code> or <code>Id</code>注解， <br> 则会将属性本身的名字作为与表的映射名称 2.
   * TODO 默认值设置 3. TODO VARCHAR，int类型自定义长度设置
   *
   * @param clazz 类
   */
  private void parseClassMap(Class<?> clazz) {
    ...
    for (Field field : fields) {
      ...
      if (id != null) {
        ...
        columns.add(id.value().toUpperCase() + " " + id.jdbcType()
            + "(255) DEFAULT '' PRIMARY KEY NOT NULL");
      } else if (column != null) {
        ClassMap.Property property = classMap.new Property(field.getName(),
            field.getType(),
        ...
        columns.add(
            CustomStringUtils.humpToUnderline(field.getName()) + " " + "VARCHAR(1500) DEFAULT ''");
      }
    }
